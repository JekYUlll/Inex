...
const signatures = residueSignatures(canary, [originalPlaintext, dirtyPlaintext]);
...
try {
  await prepareFixture(fixture, originalPlaintext);
...
async function prepareFixture(paths, plaintext) {
...
async function importVault(paths, inexPath, signatures, redactions) {
  ...
  const result = await runChild(
    inexPath,
    ["import", paths.sourcePath, paths.vaultPath],
    {
  ...
  assertCapturesHaveNoResidue("inex import", result.stdout, result.stderr);
  if (result.code !== 0) {
    throw new Error(
      `inex import failed (${result.code}): ${redact(result.stdout.text(), redactions)}\n${redact(result.stderr.text(), redactions)}`,
    );
  }
  const vault = await fs.lstat(paths.vaultPath);
  if (!vault.isDirectory() || vault.isSymbolicLink()) {
    throw new Error("inex import did not publish a regular vault directory");
  }
}
...
async function runStage(
  ...
  const testOptions = {
    ...
    extensionTestsEnv: {
      ...
      INEX_TEST_INEXD_PATH: runnerOptions.inexdPath,
