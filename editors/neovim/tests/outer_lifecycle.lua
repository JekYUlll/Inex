...
assert(vim.bo[buffer].modifiable == false, "Outer MVP buffer must be read-only")
assert(not vim.bo[buffer].modified, "Inex buffer must be clean after opening")
assert(vim.deep_equal(vim.api.nvim_buf_get_lines(buffer, 0, -1, false), { "", "" }), "new document must be blank")
