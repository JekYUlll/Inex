...
- `:InexManagePrivateAnnotationProfiles` creates, edits, removes, and sets or
  clears encrypted default profiles. The repeated picker controls kind, active
  tags, and Outer strategy; it stores no profile data locally and never asks
  for instance cover text while editing a reusable profile.
