...
- CAS v4 最终三路独立复核均给出当前 Linux 源码 checkpoint GO：安全、测试/文档与跨平台契约合计 blocker/confirmed-major/required-minor 为 0/0/0；一致判定 GA/native Windows 仍 NO-GO。全 workspace 本机测试、fmt、pedantic Clippy、rustdoc warnings-as-errors，以及 Windows GNU 全 workspace check/pedantic Clippy 和固定 actionlint 已通过；提交前将从最终差异再跑完整 workspace 与仓库卫生门禁。
