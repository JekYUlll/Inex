...
- 2026-07-14 raw index底层证明已固定为`62fa0aa`：parser直接处理SHA-1 index v2/v3/v4，拒绝非stage-0、非`100644`、semantic flags、zero OID、非UTF-8/不安全path、非canonical v4 varint、错误排序/填充/trailer及危险/未知extension；只允许唯一的TREE/REUC/UNTR/FSMN/EOIE/IEOT。真实Git 2.43证明IEOT不要求EOIE，现已接受该合法组合，同时保持IEOT partition/block严格独立解码。
