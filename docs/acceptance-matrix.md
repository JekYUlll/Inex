...
| Repository snapshot import | Clean/dirty SHA-1 HEAD, index/worktree/control mutation, filters/LFS/submodule/link/hardlink, Markdown+25,074,521-byte asset, resource boundaries, target-object plaintext scan, every construction/durability/publication kill edge, retry/reconcile, Linux and native Windows | Source history and worktree remain byte/identity/semantic identical; target is absent, a retained explicit candidate, indeterminate, or one complete new parentless ciphertext repository with stable terminal fields. Linux trusted-local functional coverage alone is not GA evidence. |
