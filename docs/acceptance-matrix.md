...
| KDF resources | Synthetic fast/slow/discrete calibration, absent/explicit RPC KDF, below/above creation cap, valid older weak vault, stronger authenticated slot rewrap | Default creation selects ops 3–20 at fixed 64 MiB without timing user secrets; explicit RPC memory outside exact 64 MiB fails before root creation; reader ceiling remains broader; add/change never lowers either authenticated work-factor component. |
