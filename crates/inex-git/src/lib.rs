...
pub use repository_import::candidate_transaction::{
    ExistingRepositoryCandidatePreview, PublishedRepositoryCandidate,
    RepositoryCandidatePublicationFailure, RepositoryCandidatePublicationFailureKind,
    initial_repository_publication_supported, preview_existing_repository_candidate,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum DurableJournalCheckpointV5 {
    ScratchCreated,
    JournalWritten,
    BeforeMove,
    CriticalAudit,
...
#[allow(
    dead_code,
    reason = "fault-injection tests inspect the complete durable-journal boundary"
)]
struct DurableJournalContextV5<'a> {
    root: &'a Path,
    local: &'a Path,
...
trait DurableJournalHooksV5 {
    fn next_token(&mut self) -> String;

    fn checkpoint(
        &mut self,
        _checkpoint: DurableJournalCheckpointV5,
...
struct ProductionDurableJournalHooksV5;

impl DurableJournalHooksV5 for ProductionDurableJournalHooksV5 {
    fn next_token(&mut self) -> String {
...
fn read_v5_prejournal_state_from_disk(
    guard: &VaultMutationGuard,
    root: &Path,
) -> Result<Option<V5PreJournalState>, GitError> {
...
fn load_v5_reference_from_disk(
    root: &Path,
) -> Result<candidate_bundle_v5::CandidateBundleTransactionReferenceV5, GitError> {
    let namespace = candidate_bundle_v5::inspect_candidate_bundle_namespace_v5(root)?;
    let stable_basename = namespace
        .stable_bundle_basename
        .as_deref()
        .ok_or(GitError::RecoveryConflict)?;
    let inventory =
        candidate_bundle_v5::validate_candidate_bundle_inventory_v5(root, stable_basename, None)?;
    let reference = candidate_bundle_v5::candidate_bundle_transaction_reference_v5(
        stable_basename,
        inventory.manifest.object_format,
        inventory.manifest_reference.clone(),
    ...
    M: FnOnce(&Path, &File, &Path) -> io::Result<AtomicFileMoveOutcome>,
{
    ...
    if source_absent && receipt_matches {
        after_move?;
        return Ok(move_result.unwrap_or(ParentSyncStatus::NotSynced));
    }
...
///
/// Every successful transition is discarded and then reclassified from the
/// filesystem. This function deliberately stops at `JournalReady`; it never
/// changes a worktree file, candidate index, live index, or cleanup namespace.
fn advance_v5_to_durable_journal(
    vault: &Vault,
    git: &Git,
    guard: &VaultMutationGuard,
    ...
    for _ in 0..3 {
        let expected = match state {
            V5PreJournalState::StableOnly => {
                ...
                let inventory =
                    candidate_bundle_v5::load_candidate_bundle_for_git_v5(guard, git, &reference)?;
            ...
            V5PreJournalState::PublishReady => {
                ...
                authorize_payload_before_v5_journal(
                    ...
                    git,
                    guard,
                    &loaded.inventory.manifest.transaction,
                )?;
                candidate_bundle_v5::acquire_index_lock_marker_v5(
                    guard,
                    git,
                    &reference,
                    &loaded.inventory,
            ...
            V5PreJournalState::MarkerNoJournal => {
                ...
                let loaded =
                    candidate_bundle_v5::load_candidate_publish_staging_v5(guard, git, &reference)?;
                let marker = candidate_bundle_v5::load_acquired_index_lock_marker_v5(
                    guard,
                    git,
                    &reference,
                    &loaded.inventory,
            ...
            V5PreJournalState::LiveIndexDrift => return Err(GitError::IndexChanged),
            V5PreJournalState::ForeignLock | V5PreJournalState::Conflict => {
                return Err(GitError::RecoveryConflict);
            }
        };
        state = read_v5_prejournal_state_from_disk(guard, vault.root())?
            .ok_or(GitError::RecoveryConflict)?;
        if state != expected {
            return Err(match state {
                V5PreJournalState::LiveIndexDrift => GitError::IndexChanged,
                V5PreJournalState::StableOnly
                | V5PreJournalState::PublishReady
                | V5PreJournalState::MarkerNoJournal
                | V5PreJournalState::JournalReady
                | V5PreJournalState::ForeignLock
                | V5PreJournalState::Conflict => GitError::RecoveryConflict,
            });
        }
    }
    if state == V5PreJournalState::JournalReady {
        Ok(state)
    } else {
        Err(GitError::RecoveryConflict)
    }
...
fn audit_durable_journal_before_move_v5(
    vault: &Vault,
    git: &Git,
    guard: &VaultMutationGuard,
    reference: &candidate_bundle_v5::CandidateBundleTransactionReferenceV5,
    inventory: &candidate_bundle_v5::InventoryVerifiedCandidateBundleV5,
    publish: &candidate_bundle_v5::PreparedCandidatePublishStagingV5,
    marker: &candidate_bundle_v5::AcquiredIndexLockMarkerV5,
    scratch_path: &Path,
    scratch_file: &File,
    bytes: &[u8],
    journal: &BundleMergeJournalV5,
) -> Result<(), GitError> {
    validate_local_directory(&git.root.join(VAULT_LOCAL_DIRECTORY))?;
    validate_local_directory(&git.root.join(".git"))?;
    if vault.root() != git.root || !guard.is_for_root(vault.root()) {
        return Err(GitError::RecoveryConflict);
    }
    candidate_bundle_v5::revalidate_acquired_index_lock_marker_v5(
        guard, git, reference, inventory, publish, marker,
    )?;
    verify_held_durable_journal_v5(scratch_path, scratch_file, bytes, journal)?;
    if read_v5_prejournal_state_from_disk(guard, vault.root())?
        != Some(V5PreJournalState::MarkerNoJournal)
    {
        return Err(GitError::RecoveryConflict);
    }
    authorize_payload_before_v5_journal(vault, git, guard, &inventory.manifest.transaction)?;
    candidate_bundle_v5::revalidate_acquired_index_lock_marker_v5(
        guard, git, reference, inventory, publish, marker,
    )?;
    verify_held_durable_journal_v5(scratch_path, scratch_file, bytes, journal)?;
...
#[allow(clippy::too_many_arguments)]
fn reconcile_durable_journal_move_error_v5(
    git: &Git,
    scratch_path: &Path,
    journal_path: &Path,
    scratch_file: &File,
    ...
    let destination_matches =
        open_file_matches_path_and_is_single_link(journal_path, scratch_file).unwrap_or(false);
    let source_absent = path_entry_is_absent(scratch_path).unwrap_or(false);
    let destination_absent = path_entry_is_absent(journal_path).unwrap_or(false);

    if source_absent && destination_matches {
        verify_held_durable_journal_v5(journal_path, scratch_file, bytes, journal)?;
    ...
    if source_matches && destination_absent {
        return Err(io_error(GitIoOperation::WriteJournal, error));
    }
    if source_matches && !destination_absent {
        return Err(GitError::RecoveryConflict);
...
fn publish_durable_journal_v5_with_hooks<H: DurableJournalHooksV5>(
    vault: &Vault,
    git: &Git,
    guard: &VaultMutationGuard,
    reference: &candidate_bundle_v5::CandidateBundleTransactionReferenceV5,
    ...
    let journal = BundleMergeJournalV5 {
        version: 5,
        reference: reference.clone(),
        index_lock_marker: marker.marker.clone(),
    };
    let bytes = serialize_bundle_journal_v5(&journal)?;
    let local = git.root.join(VAULT_LOCAL_DIRECTORY);
    ...
    let (_scratch_basename, scratch_path, mut scratch_file) =
        candidate_bundle_v5::create_private_retained_scratch_file_v5(guard, git, &local, || {
            hooks.next_token()
        })?;
    let journal_path = journal_path(&git.root);
    let lock_path = index_lock_path(&git.root);
    let context = DurableJournalContextV5 {
        root: &git.root,
        local: &local,
    ...
    hooks.checkpoint(DurableJournalCheckpointV5::ScratchCreated, &context)?;
    ...
    sync_directory(&local).map_err(|_| GitError::DurabilityNotConfirmed)?;
    verify_held_durable_journal_v5(&scratch_path, &scratch_file, &bytes, &journal)?;
    hooks.checkpoint(DurableJournalCheckpointV5::JournalWritten, &context)?;
    hooks.checkpoint(DurableJournalCheckpointV5::BeforeMove, &context)?;
    hooks.checkpoint(DurableJournalCheckpointV5::CriticalAudit, &context)?;
    audit_durable_journal_before_move_v5(
        vault,
        git,
        guard,
        reference,
    ...
    let parents_durable = match hooks.move_journal(&scratch_path, &scratch_file, &journal_path) {
        Ok(outcome) => {
            outcome.source_parent_sync == ParentSyncStatus::Synced
                && outcome.destination_parent_sync == ParentSyncStatus::Synced
        }
        Err(error) => {
            reconcile_durable_journal_move_error_v5(
                git,
                &scratch_path,
                &journal_path,
                &scratch_file,
    ...
    hooks.checkpoint(DurableJournalCheckpointV5::AfterMove, &context)?;
    ...
    sync_directory(&local).map_err(|_| GitError::DurabilityNotConfirmed)?;
    sync_directory(&git_directory).map_err(|_| GitError::DurabilityNotConfirmed)?;
    if read_journal(&git.root)? != Some(PendingMergeJournal::BundleV5(journal.clone())) {
        return Err(GitError::RecoveryConflict);
    }
    let loaded =
        candidate_bundle_v5::load_candidate_publish_staging_with_journal_v5(guard, git, reference)?;
    candidate_bundle_v5::revalidate_candidate_publish_staging_with_journal_v5(
        guard,
        git,
        reference,
        &loaded.inventory,
    ...
    let rebound_marker = candidate_bundle_v5::load_acquired_index_lock_marker_with_journal_v5(
        guard,
        git,
        reference,
        &loaded.inventory,
    ...
    candidate_bundle_v5::revalidate_acquired_index_lock_marker_with_journal_v5(
        guard,
        git,
        reference,
        &loaded.inventory,
    ...
    hooks.checkpoint(DurableJournalCheckpointV5::PostAudit, &context)?;
    if !parents_durable {
        return Err(GitError::DurabilityNotConfirmed);
    }
    Ok(())
...
mod tests {
    ...
    fn save_test_document(root: &Path, plaintext: &[u8], modified_at_ms: i64) {
        ...
        save_test_document_at(root, &logical, plaintext, modified_at_ms);
    }

    fn save_test_document_at(
        root: &Path,
        logical: &LogicalPath,
        plaintext: &[u8],
        modified_at_ms: i64,
    ...
    fn create_conflicted_repository_with_format(
        ...
        create_conflicted_repository_with_format_and_logical(object_format, "entry.md")
    }

    fn create_conflicted_repository_with_format_and_logical(
        object_format: GitObjectFormat,
        logical_path: &str,
    ) -> (TestDirectory, Vault) {
        ...
        let logical = LogicalPath::parse_canonical(logical_path).expect("test path is valid");
        let physical_path = format!("{logical_path}.enc");
        vault
        ...
        save_test_document_at(directory.path(), &logical, b"ours\n", 1_783_699_202_000);
        ...
        save_test_document_at(directory.path(), &logical, b"theirs\n", 1_783_699_203_000);
        assert!(test_git(directory.path(), ["add", &physical_path]));
    ...
    fn create_candidate_bundle_preparation_fixture(
        ...
        create_candidate_bundle_preparation_fixture_with_logical(object_format, "entry.md")
    }

    fn create_candidate_bundle_preparation_fixture_with_logical(
        object_format: GitObjectFormat,
        logical_path: &str,
    ) -> CandidateBundlePreparationFixture {
        let (directory, vault) =
            create_conflicted_repository_with_format_and_logical(object_format, logical_path);
    ...
    enum CandidatePayloadKindV5 {
        InPlace,
        DetectedRename,
        SplitRename,
    }

    fn create_candidate_payload_fixture_v5(
        object_format: GitObjectFormat,
        kind: CandidatePayloadKindV5,
    ) -> CandidateBundlePreparationFixture {
        match kind {
            CandidatePayloadKindV5::InPlace => {
                create_candidate_bundle_preparation_fixture(object_format)
            }
            CandidatePayloadKindV5::DetectedRename => {
                let DetectedRenameRecoveryFixture {
                    _directory: directory,
                    vault,
                    git,
                    journal,
                ...
                CandidateBundlePreparationFixture {
                    ...
                    vault,
                    git,
                    transaction: MergeJournalPayload::DetectedRename(journal),
                }
            }
            CandidatePayloadKindV5::SplitRename => {
                let RenameRecoveryFixture {
                    _directory: directory,
                    vault,
                    git,
                    journal,
                ...
                CandidateBundlePreparationFixture {
                    ...
                    vault,
                    git,
                    transaction: MergeJournalPayload::Rename(journal),
                }
            }
        }
    }
    ...
    #[derive(Debug)]
    struct DurableJournalTestHooksV5 {
        tokens: VecDeque<String>,
        action: Option<DurableJournalTestActionV5>,
        action_fired: bool,
        retained_path: Option<PathBuf>,
    }

    impl DurableJournalTestHooksV5 {
        fn new(action: Option<DurableJournalTestActionV5>) -> Self {
            Self {
                tokens: VecDeque::new(),
                action,
                action_fired: false,
    ...
    impl DurableJournalHooksV5 for DurableJournalTestHooksV5 {
        fn next_token(&mut self) -> String {
            self.tokens
                .pop_front()
                .unwrap_or_else(|| Uuid::new_v4().simple().to_string())
        ...
        #[allow(
            clippy::too_many_lines,
            reason = "keep every journal fault mutation in one auditable checkpoint table"
        )]
        fn checkpoint(
            &mut self,
            checkpoint: DurableJournalCheckpointV5,
            context: &DurableJournalContextV5<'_>,
        ) -> Result<(), GitError> {
            let Some(action) = self.action else {
                return Ok(());
            };
            ...
            let matches = match action {
                DurableJournalTestActionV5::FailAt(expected) => checkpoint == expected,
                ...
                | DurableJournalTestActionV5::PublishRebind
                | DurableJournalTestActionV5::MarkerRebind
                | DurableJournalTestActionV5::LiveIndexDrift
                | DurableJournalTestActionV5::WorktreeDrift
                | DurableJournalTestActionV5::AttributesDrift
                | DurableJournalTestActionV5::ForeignReserved => {
                    checkpoint == DurableJournalCheckpointV5::CriticalAudit
                }
                DurableJournalTestActionV5::MoveThenError
                | DurableJournalTestActionV5::MoveErrorBeforeMove
                | DurableJournalTestActionV5::ForeignDuringMove
                | DurableJournalTestActionV5::MoveWithUnsyncedParent => false,
            };
            if !matches {
                return Ok(());
            }
            self.action_fired = true;
            match action {
                DurableJournalTestActionV5::FailAt(_) => {
                    return Err(GitError::DurabilityNotConfirmed);
                }
                DurableJournalTestActionV5::PartialScratch => {
                    fs::write(context.scratch_path, b"{\"version\":5")
                        .map_err(|_| GitError::DurabilityNotConfirmed)?;
                ...
                DurableJournalTestActionV5::ScratchTamper => {
                    let mut bytes = fs::read(context.scratch_path)
                        .map_err(|_| GitError::DurabilityNotConfirmed)?;
                    bytes[0] ^= 1;
                    fs::write(context.scratch_path, bytes)
                        .map_err(|_| GitError::DurabilityNotConfirmed)?;
                }
                DurableJournalTestActionV5::ScratchRebind => {
                    let held = context.root.join(format!(
                        "held-v5-journal-scratch-{}",
                        Uuid::new_v4().simple()
                    ));
                    let bytes = fs::read(context.scratch_path)
                        .map_err(|_| GitError::DurabilityNotConfirmed)?;
                ...
                DurableJournalTestActionV5::StableRebind => {
                    let clone = context.root.join(format!(
                        "foreign-journal-stable-{}",
                        Uuid::new_v4().simple()
                    ));
                    let held = context
                        .root
                        .join(format!("held-journal-stable-{}", Uuid::new_v4().simple()));
                    fs::create_dir(&clone).map_err(|_| GitError::DurabilityNotConfirmed)?;
                    for member in [
                        candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5,
                        candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5,
                ...
                DurableJournalTestActionV5::PublishRebind => {
                    let held = context
                        .root
                        .join(format!("held-journal-publish-{}", Uuid::new_v4().simple()));
                    let bytes = fs::read(context.publish_path)
                        .map_err(|_| GitError::DurabilityNotConfirmed)?;
                    fs::rename(context.publish_path, held)
                        .and_then(|()| fs::write(context.publish_path, bytes))
                ...
                DurableJournalTestActionV5::MarkerRebind => {
                    let held = context
                        .root
                        .join(format!("held-journal-marker-{}", Uuid::new_v4().simple()));
                    let bytes = fs::read(context.lock_path)
                        .map_err(|_| GitError::DurabilityNotConfirmed)?;
                    fs::rename(context.lock_path, &held)
                        .and_then(|()| fs::write(context.lock_path, bytes))
                        .map_err(|_| GitError::DurabilityNotConfirmed)?;
                    self.retained_path = Some(held);
                }
                DurableJournalTestActionV5::LiveIndexDrift => {
                    let path = index_path(context.root);
                    let mut bytes =
                        fs::read(&path).map_err(|_| GitError::DurabilityNotConfirmed)?;
                    bytes[0] ^= 1;
                    fs::write(path, bytes).map_err(|_| GitError::DurabilityNotConfirmed)?;
                }
                DurableJournalTestActionV5::WorktreeDrift => {
                    fs::write(
                        context.root.join("entry.md.enc"),
                        b"foreign ciphertext worktree owner",
                    )
                    .map_err(|_| GitError::DurabilityNotConfirmed)?;
                }
                DurableJournalTestActionV5::AttributesDrift => {
                    fs::write(
                        context.root.join(GIT_ATTRIBUTES_FILE),
                        b"*.md.enc text diff\n",
                    )
                    .map_err(|_| GitError::DurabilityNotConfirmed)?;
                }
                DurableJournalTestActionV5::ForeignReserved => {
                    fs::write(
                        context.local.join(PRELOCK_RESERVATION_FILE),
                        b"foreign reserved v4 owner",
                    )
                    .map_err(|_| GitError::DurabilityNotConfirmed)?;
                }
                DurableJournalTestActionV5::MoveThenError
                | DurableJournalTestActionV5::MoveErrorBeforeMove
                | DurableJournalTestActionV5::ForeignDuringMove
                | DurableJournalTestActionV5::MoveWithUnsyncedParent => unreachable!(),
            }
            Ok(())
        }
        ...
        fn move_journal(
            ...
            match self.action {
                Some(DurableJournalTestActionV5::MoveThenError) => {
                    self.action_fired = true;
                    atomic_move_verified_file_no_replace(source, source_file, destination)?;
                    Err(io::Error::other("injected error after journal move"))
                }
                Some(DurableJournalTestActionV5::MoveErrorBeforeMove) => {
                    self.action_fired = true;
                    Err(io::Error::other("injected error before journal move"))
                }
                Some(DurableJournalTestActionV5::ForeignDuringMove) => {
                    self.action_fired = true;
                    fs::write(destination, b"foreign journal owner")?;
                    Err(io::Error::new(
                        io::ErrorKind::AlreadyExists,
                        "injected competing journal",
                    ))
                }
                Some(DurableJournalTestActionV5::MoveWithUnsyncedParent) => {
                    self.action_fired = true;
                    atomic_move_verified_file_no_replace(source, source_file, destination)?;
    ...
    fn prepare_test_durable_journal_state(
        fixture: &CandidateBundlePreparationFixture,
    ) -> (
        candidate_bundle_v5::PreparedCandidateBundleV5,
        candidate_bundle_v5::LoadedCandidatePublishStagingV5,
        candidate_bundle_v5::AcquiredIndexLockMarkerV5,
    ) {
        let (prepared, publish) = prepare_test_marker_state(fixture);
        let guard = VaultMutationGuard::acquire(fixture.vault.root())
            .expect("journal marker guard acquires");
        candidate_bundle_v5::acquire_index_lock_marker_v5(
            &guard,
            &fixture.git,
            &prepared.transaction_reference,
        ...
        .expect("journal marker acquires");
        drop(publish);
        ...
        .expect("fresh journal publish state loads");
        let marker = candidate_bundle_v5::load_acquired_index_lock_marker_v5(
            &guard,
            &fixture.git,
            &prepared.transaction_reference,
        ...
        .expect("fresh journal marker proof loads");
            ...
            for (old, expected) in edges {
                ...
                match outcome {
                    FaultOutcome::ErrorBefore => {
                        ...
                        let advanced = advance_test_cleanup_one(&fixture, unchanged)
                            .expect("retry after error-before advances one adjacent edge");
                        assert_eq!(advanced.kind(), expected);
                    }
                    FaultOutcome::ErrorAfter => {
                        assert!(matches!(
                            result,
    ...
    #[allow(
        clippy::too_many_lines,
        reason = "keep the seven-state illegal artifact cross-product auditable in one table"
    )]
    fn v5_cleanup_classifier_rejects_illegal_artifact_cross_products_at_every_state() {
        #[derive(Clone, Copy, Debug)]
        enum ExtraArtifact {
            Stable,
            Cleanup,
        ...
        let wrong_token = if reference.token == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" {
            "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
        } else {
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        };
        let wrong_cleanup_basename =
            candidate_bundle_v5::candidate_bundle_cleanup_basename_v5(wrong_token)
                .expect("wrong-token cleanup basename derives");
        let wrong_cleanup_path =
            candidate_bundle_v5::candidate_bundle_cleanup_path_v5(root, &wrong_cleanup_basename)
                .expect("wrong-token cleanup path derives");
        let wrong_receipt_basename =
            candidate_bundle_v5::candidate_bundle_cleanup_receipt_basename_v5(wrong_token)
                .expect("wrong-token receipt basename derives");
        let wrong_receipt_path = candidate_bundle_v5::candidate_bundle_cleanup_receipt_path_v5(
            root,
            &wrong_receipt_basename,
        )
        .expect("wrong-token receipt path derives");
        let journal_bytes = fs::read(journal_path(root)).expect("v5 journal snapshots");
        let candidate_bytes =
            fs::read(stable_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5))
                .expect("candidate bytes snapshot");
        let manifest_bytes =
            fs::read(stable_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5))
                .expect("manifest bytes snapshot");
        ...
        for legal in states {
            let held =
                inspect_held_v5_cleanup_state(root).expect("legal cross-product state loads");
            assert_eq!(held.kind(), legal);
            drop(held);
            let exact_extras: &[ExtraArtifact] = match legal {
                V5CleanupState::StableJ => &[
                    ExtraArtifact::Cleanup,
                    ExtraArtifact::Receipt,
                    ExtraArtifact::IndexLock,
                ],
                V5CleanupState::CleanupFullJ => &[
                    ExtraArtifact::Stable,
                    ExtraArtifact::Receipt,
                    ExtraArtifact::IndexLock,
                ],
                V5CleanupState::CleanupFullR
                | V5CleanupState::CleanupManifestR
                | V5CleanupState::CleanupEmptyR
                | V5CleanupState::ReceiptOnly => &[ExtraArtifact::Stable, ExtraArtifact::Journal],
                V5CleanupState::Clean => &[
                    ExtraArtifact::Stable,
                    ExtraArtifact::Cleanup,
                    ExtraArtifact::Journal,
                ],
            };
            for artifact in exact_extras.iter().copied().chain([
                ExtraArtifact::WrongCleanup,
                ExtraArtifact::WrongReceipt,
                ExtraArtifact::Publish,
            ]) {
                match artifact {
                    ExtraArtifact::Stable => {
                        fs::create_dir(&stable_path).expect("illegal stable artifact creates");
                    }
                    ExtraArtifact::Cleanup => {
                        fs::create_dir(&cleanup_path).expect("illegal cleanup artifact creates");
                    }
                    ExtraArtifact::Journal => fs::write(journal_path(root), &journal_bytes)
                        .expect("illegal active journal creates"),
                    ExtraArtifact::Receipt => {
                        fs::write(&receipt_path, &journal_bytes).expect("illegal receipt creates");
                    }
                    ExtraArtifact::WrongCleanup => {
                        fs::create_dir(&wrong_cleanup_path).expect("wrong-token cleanup creates");
                    }
                    ExtraArtifact::WrongReceipt => fs::write(&wrong_receipt_path, &journal_bytes)
                        .expect("wrong-token receipt creates"),
                    ExtraArtifact::Publish => fs::write(&publish_path, b"illegal publish residue")
                        .expect("publish residue creates"),
                    ExtraArtifact::IndexLock => {
                        fs::write(index_lock_path(root), b"illegal Git index lock")
                            .expect("index lock residue creates");
                    }
                }
                assert!(
                    inspect_held_v5_cleanup_state(root).is_err(),
                    "illegal artifact {artifact:?} must reject legal state {legal:?}"
                );
                match artifact {
                    ExtraArtifact::Stable => {
                        fs::remove_dir(&stable_path).expect("illegal stable removes");
                    }
                    ExtraArtifact::Cleanup => {
                        fs::remove_dir(&cleanup_path).expect("illegal cleanup removes");
                    }
                    ExtraArtifact::Journal => {
                        fs::remove_file(journal_path(root)).expect("illegal journal removes");
                    }
                    ExtraArtifact::Receipt => {
                        fs::remove_file(&receipt_path).expect("illegal receipt removes");
                    }
                    ExtraArtifact::WrongCleanup => {
                        fs::remove_dir(&wrong_cleanup_path).expect("wrong-token cleanup removes");
                    }
                    ExtraArtifact::WrongReceipt => {
                        fs::remove_file(&wrong_receipt_path).expect("wrong-token receipt removes");
                    }
                    ExtraArtifact::Publish => {
                        fs::remove_file(&publish_path).expect("publish residue removes");
                    }
                    ExtraArtifact::IndexLock => {
                        fs::remove_file(index_lock_path(root)).expect("index lock residue removes");
                    }
                }
                assert_eq!(
                    inspect_held_v5_cleanup_state(root)
                        .expect("legal state restores after illegal artifact removal")
                        .kind(),
                    legal
                );
            }

            if legal == V5CleanupState::CleanupFullJ {
                let candidate_path =
                ...
                fs::remove_file(&candidate_path).expect("candidate removes for manifest-plus-J");
                assert!(inspect_held_v5_cleanup_state(root).is_err());
                fs::write(&candidate_path, &candidate_bytes).expect("candidate restores");
                fs::remove_file(&candidate_path).expect("candidate removes for empty-plus-J");
                fs::remove_file(&manifest_path).expect("manifest removes for empty-plus-J");
                assert!(inspect_held_v5_cleanup_state(root).is_err());
                fs::write(&candidate_path, &candidate_bytes)
                    .expect("empty-plus-J candidate restores");
                fs::write(&manifest_path, &manifest_bytes).expect("empty-plus-J manifest restores");
                assert_eq!(
                    inspect_held_v5_cleanup_state(root)
                        .expect("full-plus-J state restores")
                        .kind(),
                    legal
                );
            }

            if legal != V5CleanupState::Clean {
                let current = inspect_held_v5_cleanup_state(root)
                    .expect("cross-product transition reloads held proof");
                let advanced = advance_test_cleanup_one(&fixture, current)
                    .expect("cross-product fixture advances one edge");
                drop(advanced);
            }
        }

        for member_count in [0_u8, 1, 2] {
            fs::create_dir(&cleanup_path).expect("orphan cleanup stage creates");
            if member_count >= 1 {
                fs::write(
                    cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5),
                    &manifest_bytes,
                )
                .expect("orphan manifest creates");
            }
            if member_count == 2 {
                fs::write(
                    cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5),
                ...
                .expect("orphan candidate creates");
            }
            assert!(inspect_held_v5_cleanup_state(root).is_err());
            if member_count == 2 {
                fs::remove_file(cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5))
                    .expect("orphan candidate removes");
            }
            if member_count >= 1 {
                fs::remove_file(
                    cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5),
                )
                .expect("orphan manifest removes");
            }
            fs::remove_dir(&cleanup_path).expect("orphan cleanup stage removes");
        }
        assert_eq!(
            inspect_held_v5_cleanup_state(root)
                .expect("cross-product fixture ends clean")
                .kind(),
            V5CleanupState::Clean
        );
    }
    ...
    fn v5_cleanup_full_j_reauthorizes_completed_index_and_worktree_before_receipt() {
        for drift in ["index", "worktree", "protected-alias"] {
            let fixture = create_candidate_payload_fixture_v5(
                GitObjectFormat::Sha1,
                CandidatePayloadKindV5::InPlace,
            );
            ...
            if drift == "worktree" {
                ...
                let cache_info = format!(
                    "{},{},ENTRY.MD.ENC",
                    transaction.result_mode, transaction.result_oid
                );
                assert!(test_git(
                    root,
    ...
    #[test]
    fn v5_cleanup_full_j_rejects_relocated_unicode_protected_alias() {
        let fixture = create_candidate_bundle_preparation_fixture_with_logical(
            GitObjectFormat::Sha1,
            "Straße.md",
        );
        let root = fixture.vault.root();
        let prepared = prepare_test_cleanup_stable_j(&fixture);
        let stable = inspect_held_v5_cleanup_state(root).expect("Unicode stable state loads");
        let full_j = advance_test_cleanup_one(&fixture, stable)
            .expect("Unicode stable bundle retires to cleanup");
        assert_eq!(full_j.kind(), V5CleanupState::CleanupFullJ);
        drop(full_j);
        let MergeJournalPayload::InPlace(transaction) = &fixture.transaction else {
            panic!("fixture must carry an in-place Unicode payload");
        };
        let alias = "STRASSE.md.enc";
        assert_eq!(
            inex_core::path::raw_portable_case_fold_key(&transaction.physical_path),
            inex_core::path::raw_portable_case_fold_key(alias),
            "integration fixture must use a real Unicode portable alias"
        );
        let cache_info = format!(
            "{},{},{alias}",
            transaction.result_mode, transaction.result_oid
        ...
        let journal_before = fs::read(journal_path(root)).expect("Unicode journal snapshots");
        let state = inspect_held_v5_cleanup_state(root).expect("Unicode full J state loads");
        assert!(matches!(
            advance_test_cleanup_one(&fixture, state),
            Err(GitError::IndexChanged | GitError::RecoveryConflict)
        ));
        assert_eq!(
            inspect_held_v5_cleanup_state(root)
                .expect("Unicode alias leaves full J physically classified")
                .kind(),
            V5CleanupState::CleanupFullJ
        );
        assert_eq!(
            fs::read(journal_path(root)).expect("Unicode active journal remains"),
            journal_before
        );
        let receipt = candidate_bundle_v5::candidate_bundle_cleanup_receipt_basename_v5(
            &prepared.transaction_reference.token,
        )
        .expect("Unicode receipt basename derives");
        assert!(!root.join(VAULT_LOCAL_DIRECTORY).join(receipt).exists());
    }
    ...
    #[test]
    fn v5_cleanup_post_operation_target_rebinds_never_advance_silently() {
        let fixture = create_candidate_payload_fixture_v5(
            GitObjectFormat::Sha1,
            CandidatePayloadKindV5::InPlace,
        );
        ...
        let stable = inspect_held_v5_cleanup_state(root).expect("stable target proof loads");
        let held_cleanup = local.join("held-post-stable-cleanup-target");
        let result = advance_test_cleanup_one_with_hook(&fixture, stable, |_, _| {
            fs::rename(&cleanup_path, &held_cleanup).expect("moved cleanup target holds");
            fs::create_dir(&cleanup_path).expect("cleanup target clone creates");
            for member in [
                candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5,
                candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5,
            ] {
                fs::copy(held_cleanup.join(member), cleanup_path.join(member))
                    .expect("cleanup target member clones");
            }
            Ok(())
        });
        assert!(matches!(result, Err(GitError::RecoveryConflict)));
        fs::remove_file(cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5))
            .expect("cleanup candidate clone removes");
        fs::remove_file(cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5))
            .expect("cleanup manifest clone removes");
        fs::remove_dir(&cleanup_path).expect("cleanup directory clone removes");
        fs::rename(&held_cleanup, &cleanup_path).expect("moved cleanup target restores");

        let full_j = inspect_held_v5_cleanup_state(root).expect("full J target proof loads");
        let held_cleanup = local.join("held-post-journal-cleanup-target");
        let result = advance_test_cleanup_one_with_hook(&fixture, full_j, |_, _| {
            fs::rename(&cleanup_path, &held_cleanup).expect("full cleanup target holds");
            fs::create_dir(&cleanup_path).expect("full cleanup target clone creates");
            for member in [
                candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5,
                candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5,
            ] {
                fs::copy(held_cleanup.join(member), cleanup_path.join(member))
                    .expect("full cleanup target member clones");
            }
            Ok(())
        });
        assert!(matches!(result, Err(GitError::RecoveryConflict)));
        fs::remove_file(cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_INDEX_V5))
            .expect("full candidate clone removes");
        fs::remove_file(cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5))
            .expect("full manifest clone removes");
        fs::remove_dir(&cleanup_path).expect("full cleanup clone removes");
        fs::rename(&held_cleanup, &cleanup_path).expect("full cleanup target restores");

        let full_r = inspect_held_v5_cleanup_state(root).expect("full R target proof loads");
        let manifest_path = cleanup_path.join(candidate_bundle_v5::CANDIDATE_BUNDLE_MANIFEST_V5);
        let held_manifest = local.join("held-post-candidate-manifest-target");
        let result = advance_test_cleanup_one_with_hook(&fixture, full_r, |_, _| {
            fs::rename(&manifest_path, &held_manifest).expect("manifest target holds");
            fs::copy(&held_manifest, &manifest_path).expect("manifest target clone rebounds");
            Ok(())
        });
        assert!(matches!(result, Err(GitError::RecoveryConflict)));
        fs::remove_file(&manifest_path).expect("manifest target clone removes");
        fs::rename(&held_manifest, &manifest_path).expect("manifest target restores");

        let manifest_r = inspect_held_v5_cleanup_state(root).expect("manifest R proof loads");
        let held_empty = local.join("held-post-manifest-empty-target");
        let result = advance_test_cleanup_one_with_hook(&fixture, manifest_r, |_, _| {
            fs::rename(&cleanup_path, &held_empty).expect("empty target holds");
            fs::create_dir(&cleanup_path).expect("empty target clone rebounds");
            Ok(())
        });
        assert!(matches!(result, Err(GitError::RecoveryConflict)));
        fs::remove_dir(&cleanup_path).expect("empty target clone removes");
        fs::rename(&held_empty, &cleanup_path).expect("empty target restores");

        let empty_r = inspect_held_v5_cleanup_state(root).expect("empty R proof loads");
        let result = advance_test_cleanup_one_with_hook(&fixture, empty_r, |_, _| {
            fs::create_dir(&cleanup_path).expect("removed cleanup target rebounds");
            Ok(())
        });
        assert!(matches!(result, Err(GitError::RecoveryConflict)));
        fs::remove_dir(&cleanup_path).expect("rebound removed cleanup target removes");
        assert_eq!(
            inspect_held_v5_cleanup_state(root)
                .expect("receipt-only target state remains")
                .kind(),
            V5CleanupState::ReceiptOnly
        );
        advance_test_cleanup_to_clean(&fixture).expect("target-rebind fixture reaches clean");
    }
    ...
    fn v5_recovery_advances_all_payloads_and_object_formats_to_clean() {
        for object_format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
            for kind in [
                ...
                assert_eq!(
                    inspect_test_v5_prejournal_state(root).expect("stable state inspects"),
                    Some(V5PreJournalState::StableOnly)
                );
                assert!(matches!(
                    recover_pending(&fixture.vault, &fixture.git),
            ...
            for object_format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
                for kind in [
                    ...
                    if start_with_marker {
                        let guard = VaultMutationGuard::acquire(root)
                            .expect("fresh-entry marker guard acquires");
                        let loaded = candidate_bundle_v5::load_candidate_publish_staging_v5(
                            &guard,
                            &fixture.git,
                            &prepared.transaction_reference,
                        )
                        .expect("fresh-entry publish state loads");
                        authorize_payload_before_v5_journal(
                            &fixture.vault,
                            &fixture.git,
                            &guard,
                        ...
                        .expect("fresh-entry payload authorizes before marker");
                        candidate_bundle_v5::acquire_index_lock_marker_v5(
                            &guard,
                            &fixture.git,
                            &prepared.transaction_reference,
                        ...
                        .expect("fresh-entry marker acquires");
                    }
                    assert_eq!(
                        inspect_test_v5_prejournal_state(root)
                            .expect("fresh-entry initial state inspects"),
                        Some(if start_with_marker {
                            V5PreJournalState::MarkerNoJournal
                        } else {
                            V5PreJournalState::PublishReady
                        })
    ...
    #[allow(
        clippy::too_many_lines,
        reason = "keep pre-move, partial-write, and post-move crash checkpoints in one matrix"
    )]
    fn v5_durable_journal_fault_checkpoints_reclassify_exact_disk_state() {
        use DurableJournalCheckpointV5 as Point;

        for point in [
            Point::ScratchCreated,
            Point::JournalWritten,
            Point::BeforeMove,
            Point::CriticalAudit,
        ] {
            let fixture = create_candidate_bundle_preparation_fixture(GitObjectFormat::Sha1);
            let root = fixture.vault.root();
            let (prepared, loaded, marker) = prepare_test_durable_journal_state(&fixture);
            let guard = VaultMutationGuard::acquire(root).expect("journal guard acquires");
            let mut hooks =
                DurableJournalTestHooksV5::new(Some(DurableJournalTestActionV5::FailAt(point)));
            assert!(matches!(
                publish_durable_journal_v5_with_hooks(
                    &fixture.vault,
                    &fixture.git,
            ...
            drop(guard);
            assert_eq!(
                inspect_test_v5_prejournal_state(root).expect("pre-move state reclassifies"),
                Some(V5PreJournalState::MarkerNoJournal),
                "{point:?}"
            );
            assert!(!journal_path(root).exists());
            assert_eq!(
                fs::read(index_lock_path(root)).expect("marker remains"),
                prepared.index_lock_marker
            );
            assert_eq!(candidate_bundle_scratch_paths(root).len(), 1);
        }

        {
            ...
            let guard = VaultMutationGuard::acquire(root).expect("journal guard acquires");
            let mut hooks =
                DurableJournalTestHooksV5::new(Some(DurableJournalTestActionV5::PartialScratch));
            assert!(matches!(
                publish_durable_journal_v5_with_hooks(
                    &fixture.vault,
                    &fixture.git,
            ...
            assert_eq!(retained.len(), 1);
            assert_eq!(
                fs::read(&retained[0]).expect("partial journal scratch reads"),
                b"{\"version\":5"
            );
            #[cfg(unix)]
            {
                ...
                assert_eq!(
                    fs::metadata(&retained[0])
                        .expect("partial scratch metadata reads")
                        .permissions()
                        .mode()
                        & 0o777,
                    0o600
            ...
            assert_eq!(
                inspect_test_v5_prejournal_state(root).expect("partial state reclassifies"),
                Some(V5PreJournalState::MarkerNoJournal)
            );
            assert_eq!(
                fs::read(index_lock_path(root)).expect("marker remains"),
        ...
        for point in [Point::AfterMove, Point::PostAudit] {
            let fixture = create_candidate_bundle_preparation_fixture(GitObjectFormat::Sha1);
            let root = fixture.vault.root();
            let (prepared, loaded, marker) = prepare_test_durable_journal_state(&fixture);
            let guard = VaultMutationGuard::acquire(root).expect("journal guard acquires");
            ...
            drop(guard);
            assert_eq!(
                inspect_test_v5_prejournal_state(root).expect("post-move state reclassifies"),
                Some(V5PreJournalState::JournalReady),
                "{point:?}"
            );
            let journal_before = fs::read(journal_path(root)).expect("visible journal snapshots");
            let guard = VaultMutationGuard::acquire(root).expect("fresh journal guard acquires");
            assert_eq!(
                advance_v5_to_durable_journal(&fixture.vault, &fixture.git, &guard)
                    .expect("fresh recovery accepts journal-ready state"),
                V5PreJournalState::JournalReady
            );
            assert_eq!(
                fs::read(journal_path(root)).expect("journal is not rewritten"),
                journal_before
            );
            assert!(candidate_bundle_scratch_paths(root).is_empty());
        }
    ...
    #[test]
    fn v5_durable_journal_move_errors_distinguish_moved_not_moved_and_foreign() {
        for action in [
            DurableJournalTestActionV5::MoveThenError,
            DurableJournalTestActionV5::MoveWithUnsyncedParent,
            DurableJournalTestActionV5::MoveErrorBeforeMove,
            DurableJournalTestActionV5::ForeignDuringMove,
        ] {
            let fixture = create_candidate_bundle_preparation_fixture(GitObjectFormat::Sha256);
            let root = fixture.vault.root();
            let (prepared, loaded, marker) = prepare_test_durable_journal_state(&fixture);
            let guard = VaultMutationGuard::acquire(root).expect("journal guard acquires");
            let mut hooks = DurableJournalTestHooksV5::new(Some(action));
            let result = publish_durable_journal_v5_with_hooks(
                &fixture.vault,
                &fixture.git,
                &guard,
                &prepared.transaction_reference,
            ...
            match action {
                DurableJournalTestActionV5::MoveThenError => {
                    result.expect("moved-then-error reconciles exact held journal");
                    assert_eq!(
                        inspect_test_v5_prejournal_state(root).expect("moved state inspects"),
                        Some(V5PreJournalState::JournalReady)
                    );
                }
                DurableJournalTestActionV5::MoveWithUnsyncedParent => {
                    ...
                    assert_eq!(
                        inspect_test_v5_prejournal_state(root)
                            .expect("unsynced visible state inspects"),
                        Some(V5PreJournalState::JournalReady)
                    );
                    let bytes = fs::read(journal_path(root)).expect("unsynced journal snapshots");
                    let guard =
                        VaultMutationGuard::acquire(root).expect("fresh journal guard acquires");
                    assert_eq!(
                        advance_v5_to_durable_journal(&fixture.vault, &fixture.git, &guard)
                            .expect("fresh recovery accepts exact visible journal"),
                        V5PreJournalState::JournalReady
                    );
                    assert_eq!(fs::read(journal_path(root)).expect("journal reads"), bytes);
                }
                DurableJournalTestActionV5::MoveErrorBeforeMove => {
                    assert!(matches!(
                        result,
                    ...
                    assert_eq!(
                        inspect_test_v5_prejournal_state(root).expect("not-moved state inspects"),
                        Some(V5PreJournalState::MarkerNoJournal)
                    );
                    assert!(!journal_path(root).exists());
                    assert_eq!(candidate_bundle_scratch_paths(root).len(), 1);
                ...
                DurableJournalTestActionV5::ForeignDuringMove => {
                    assert!(matches!(result, Err(GitError::RecoveryConflict)));
                    assert_eq!(
                        fs::read(journal_path(root)).expect("foreign journal remains"),
                        b"foreign journal owner"
                    );
                    assert_eq!(candidate_bundle_scratch_paths(root).len(), 1);
                    assert!(matches!(
                        recovery_status(root),
    ...
    #[test]
    fn v5_durable_journal_critical_audit_rejects_all_rebinding_and_drift() {
        for action in [
            DurableJournalTestActionV5::ScratchTamper,
            DurableJournalTestActionV5::ScratchRebind,
            DurableJournalTestActionV5::StableRebind,
            ...
            assert!(!journal_path(root).exists(), "{action:?}");
            assert_eq!(candidate_bundle_scratch_paths(root).len(), 1, "{action:?}");
            if action == DurableJournalTestActionV5::MarkerRebind {
                let held = hooks
                    .retained_path
                    .as_ref()
                    .expect("rebound marker keeps original owner");
                assert_eq!(
                    fs::read(held).expect("held original marker reads"),
                    prepared.index_lock_marker
                );
                assert_eq!(
                    fs::read(index_lock_path(root)).expect("replacement marker remains"),
                    prepared.index_lock_marker
                );
            } else {
                assert_eq!(
                    fs::read(index_lock_path(root)).expect("canonical marker remains"),
                    prepared.index_lock_marker,
                    "{action:?}"
                );
            }
    ...
    #[test]
    fn v5_fresh_marker_loader_requires_single_link_identity() {
        let fixture = create_candidate_bundle_preparation_fixture(GitObjectFormat::Sha1);
        let root = fixture.vault.root();
        let (prepared, loaded, _marker) = prepare_test_durable_journal_state(&fixture);
        let sentinel = root.join("v5-marker-hardlink-sentinel");
        fs::hard_link(index_lock_path(root), &sentinel).expect("marker hard link creates");
        let guard = VaultMutationGuard::acquire(root).expect("fresh marker guard acquires");
        assert!(matches!(
            candidate_bundle_v5::load_acquired_index_lock_marker_v5(
                &guard,
                &fixture.git,
                &prepared.transaction_reference,
                &loaded.inventory,
