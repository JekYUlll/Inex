...
| Packaging | Current strict release-tool source tests pass 59/59. Target-bound Cargo inventories now require the exact four reviewed workspace manifests, canonical JSON, locked checksums and license-text hashes, a fixed engineering license policy, fixed libsodium ISC text, identical inventories/sidecars across all three packages, and canonical package/audit/lifecycle report schemas. Package smoke binds the compiled Rust target, release profile, and exact reviewed libsodium runtime. Earlier clean Linux x64 reproducibility/lifecycle evidence remains a historical checkpoint | The earlier final artifacts predate strict release-set evidence v1 and must be rebuilt from the new clean source checkpoint. Native Windows/arm64 runs, failure-state/two-version drills, signatures, publication, and independent legal review remain pending |
