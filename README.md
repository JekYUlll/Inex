...
|---------|------------------|--------------------|
| Rust core, CLI, daemon, import, and Git | Linux workspace tests and strict static gates pass, including authenticated detected/split rename/modify and v1/v2/v3 recovery states; Windows GNU compiles and substantial Wine tests pass | Native Windows/MSVC filesystem and crash/power-loss evidence remain pending; concurrent external Git porcelain is unsupported because the final index update has no cross-process CAS |
...
| Packaging | Release-tool tests pass 49/49, `actionlint` and pedantic/all-features Clippy pass, and independent artifact-tool code review is GO. A clean-source system-GCC Linux x64 double build is byte-identical and passes strict archive/native-dependency audit plus VS Code 1.125.0 CLI install/bundled-sidecar smoke. A clean standalone lifecycle run at harness commit `1e01842` binds the final Linux x64 artifacts to import/password/Git-bundle/clean-tree-copy restore, frozen-v1 compatibility, exact body comparison, and zero sensitive-residue hits outside the designated plaintext source | Native Windows/arm64 workflow runs, failure-state/two-version drills, signatures, publication, and independent legal review remain pending |
