...
| Sublime Text | 72/72 Python tests pass: 61 product tests plus 11 runner/evidence tests. On Linux, separately preserved canonical reports bind one exact packaged Build 4200 normal scenario and one exact packaged plugin-host SIGKILL scenario; the normal flow covers unlock/open/edit/save/close and CRUD through real panels, while the crash flow reproduces the host-dead copy boundary | The plugin-host SIGKILL leaves the visible buffer copyable, cannot restart the host in-process, and requires a full Sublime restart. That is boundary evidence, not plaintext-erasure success. Same-profile restart and the remaining persistent-profile/full-platform matrix are pending, so the client remains experimental |
