...
- [ ] 修复 CustomEditor 的 CRLF 展示/同步边界：导航、selection 与 snapshot 不得将浏览器 textarea 的 LF 规范化误认作用户编辑；补充真实加密 Git worktree 无操作不变回归
  - [x] 隔离 Extension Host fixture 已以真实 CRLF Markdown 导入密文 Git vault，执行 open/hide/reveal/close 后强制 `git status --porcelain=v1 -z` 为空；普通 textarea LF presentation 不再能形成无编辑 `.enc` 修改（本轮）。用户持久 profile 的人工复验仍待完成。
- [ ] 在不降级 CustomEditor plaintext-residue 边界的前提下，实现 webview 内 Markdown 语法呈现并修复 Headings 的可重复跳转；普通 VS Code SCM 保持密文 binary diff，另行设计需解锁的安全 revision-compare UX
  ...
  - 2026-07-16: `Inex: Compare HEAD with Parent (Outer)` 已加入 VS Code Source Control 的 `*.md.enc` resource context menu；命令只接受 Extension Host `Uri`/tree node 或 clean editor，控制器重新验证 vault-root/canonical logical path，仍固定 HEAD/first-parent 并使用无脚本 Inex compare panel。Extension Host 已走 URI resource path 并要求第二条 authenticated compare trace。
  - 2026-07-16: `b68d745` standalone clean-source Linux x64 package/release-set audit 已完成并覆盖安装，包含 Source Control secure-compare 入口。
...
- [x] 实现 Umbra RenderMap mutation 基础：只转发 daemon 已认证的投影/ETag/RenderMap，支持 apply/remove 后整体替换 projection；空 tag IDs 与 PRD 一致，转换/锁定时不保留该私密状态
...
- [ ] 实现 Umbra 标签/profile 选择 UI；不得绕过 Outer/Umbra 隔离
  - [x] Neovim live encrypted profile picker/apply commands（含一次性公开 cover text）
  - [x] Neovim custom kind/tag 多选 picker（headless state-machine regression 覆盖 kind/Outer 单选、multi-tag canonicalization、archived tag 排除与取消）
  - [x] Neovim tag/profile 管理 UI
    - [x] Neovim encrypted tag create/rename/archive/reorder UI（每次 mutation 后重新读取 live catalog）
    - [x] Neovim annotation-profile management UI
...
- [ ] 使用 headless Neovim 回归验证，并对 swap、shada、undo、LSP 等宿主明文残留执行显式门控（当前 transport/Outer/Umbra lifecycle 已通过；完整 host-residue gate 仍未完成）
  - 2026-07-16: fresh restricted temporary vault 下的 `outer_lifecycle.lua` 已以 absolute local `inexd` 完整通过：Outer/Umbra unlock/lock、create/save/search/mkdir、lock/reopen、feature-2 enable 与 annotation mutation 都经真实 sidecar；临时 vault 已删除。该回归不替代 shada/LSP/third-party host residue gate。
  - 2026-07-16: Linux `tests/run_outer_lifecycle.sh` 已将上述临时-vault setup 固化为可重复 runner；只接受 absolute `INEX_SIDECAR` 并在退出时删除 fixture。
...
### Phase 6: Git 合并、迁移与恢复工具
  ...
  - [ ] 设计并实现显式 Plaintext Export Mode：统一由已认证 `inexd` 执行，所有编辑器仅调用同一 RPC；默认关闭、无密码绕过、拒绝导出到 vault 内或已存在目标，使用 staging→审计→atomic publish，并把 plaintext-residue/备份/Git 风险作为不可消除的确认语义
    ...
    - [x] 实现仅 Outer 的 feature-2 Markdown projection：公开 drop/cover/placeholder 策略替换 marker，严格拒绝不一致 marker/slot/cover，且 API 不读取 `K_umbra` 或私密 payload（本轮）
    ...
    - [ ] CLI/VS Code 首先接入，Sublime/Neovim 仅复用 RPC 与同一确认语义
      ...
      - [x] 隔离 Extension Host 现真实调用 VS Code Outer export transaction，核验 Markdown/asset 输出与 prepare→commit trace 顺序，并在残留审计前删除授权测试副本（本轮）
      - [x] 隔离 Extension Host 已在 live Umbra session 中创建 private slot 后执行 VS Code `scope=umbra` prepare→commit，确认导出包含 canonical private block、kind metadata 和私密 Markdown；导出目录在 residue audit 前删除（本轮）
      - [x] VS Code export destination UI 仅接受一个非 `.`/`..` 的无分隔符、无控制字符目录组件，不能绕过用户选择的 parent（62/62 unit + Extension Host 回归，本轮）
  ...
  - [x] 扩展daemon session/RPC及VS Code QuickPick/命令/可配置keybindings；所有私密 catalog/profile/selection 仍只经已解锁会话与 authenticated RenderMap/ETag 边界传递
    ...
    - [x] Outer `search.query` 对 feature-2 文档仅索引认证后的 public Outer projection：Drop/Cover/Placeholder 可见内容保持可搜，private Markdown/tag canary 不会进入 Outer index 或结果（本轮）。
    - [x] Umbra-private search 使用独立的 memory-only `umbra.search.query`：仅 live `K_umbra` 可构建/查询 full Umbra projections，锁定或任何文档 mutation 都清零；VS Code `Inex: Search Umbra Private Content` 通过正常命令贡献、独立密码 gate 和受控 result reveal 接入（本轮）。
  ...
  - [x] 扩展Sublime stateful picker、profile命令和keymap示例；维持 experimental 客户端与 host-residue 边界，不扩大平台安全声明
    ...
    - [x] Sublime model 已支持仅 clean normal document → Umbra projection 的显式 transition，交还 old normal handle 后置关闭（`bdf365b`）；UI transaction 待接线
    - [x] Sublime 已提供 active document 的 convert→projection-open→transition→normal-handle-close 命令；conversion 后失败 fail closed lock/scrub（`4a30a99`）
